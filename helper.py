...
# --- Tavily safety guard (prevents 400-char query errors) ---
MAX_TAVILY_QUERY_LEN = 400

def safe_tavily_query(q: str, max_len: int = MAX_TAVILY_QUERY_LEN) -> str:
    """Normalize whitespace and hard-cap query length for Tavily."""
    q = " ".join((q or "").split())
    return q[:max_len]
...
class ewriter():
    ...
    def research_plan_node(self, state: AgentState):
        ...
        content = list(state.get('content') or [])  # add to content
...
class writer_gui( ):
    def __init__(self, graph, share=False):
        ...
        self.threads = []
        self.thread_id = -1
        self.thread_ts_map = {str(self.thread_id): str(time.time())}
        self.thread = {"configurable": {"thread_id": str(self.thread_id), "thread_ts": self.thread_ts_map[str(self.thread_id)]}}
        #self.sdisps = {} #global    
        self.demo = self.create_interface()

    def run_agent(self, start, topic, stop_after):
        # Start a new run (new thread) or continue the existing one.
        if start:
            # Reset the visible output for a clean new run.
            self.partial_message = ""
            self.iterations.append(0)

            # IMPORTANT: Initialize keys the graph/nodes expect (prevents KeyError).
            config = {
                "task": topic,
                "max_revisions": 2,
                "revision_number": 1,
                ...
                # UI display helpers (safe defaults)
                "lnode": "",
                "draft": "no draft",
            ...
            # Track a stable timestamp per thread (used by the UI history dropdown).
            self.thread_ts_map[str(self.thread_id)] = str(time.time())
        else:
            config = None
            # Ensure we have a thread_ts for the current thread_id.
            self.thread_ts_map.setdefault(str(self.thread_id), str(time.time()))

        # Always include both thread_id and thread_ts in the configurable config.
        self.thread = {
            "configurable": {
                "thread_id": str(self.thread_id),
                "thread_ts": self.thread_ts_map.get(str(self.thread_id), ""),
            }
        }
    ...
    def update_hist_pd(self,):
        ...
        for state in self.graph.get_state_history(self.thread):
            if state.metadata['step'] < 1:
                continue
            thread_ts = state.config['configurable'].get('thread_ts','')
    ...
    def copy_state(self,hist_str):
        ...
        new_thread_ts = new_state.config['configurable'].get('thread_ts','')
            ...
            def updt_disp():
                ...
                for state in self.graph.get_state_history(self.thread):
                    if state.metadata['step'] < 1:  #ignore early states
                        continue
                    s_thread_ts = state.config['configurable'].get('thread_ts','')
